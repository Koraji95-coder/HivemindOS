...
AgentBase 🔁
────────────────────────────
Base class for all agents in HivemindOS.
Defines the shared interface and behavior expected from every agent
(Daphne, Bart, Cortexa, etc).

Each agent subclass should implement:
- `ask(prompt)` → sends input for processing
- `respond(input)` → returns agent-specific reply
"""
