...
version_bump.py 🛠️
────────────────────────────────────────────
Auto-bumps version, builds changelog, commits, tags, and pushes to Git.
"""
...
ROOTS = {
    ...
    "ui": Path("hivemind-ui/src/version.js"),
    "npm": Path("hivemind-ui/package.json")
}
...
def update_tasks_version(new_ver: str):
    ...
    updated = []
    for line in content:
        if '"hivemind_version":' in line:
            updated.append(f'  "hivemind_version": "v{new_ver}",  // 🔄 DO NOT REMOVE – gets auto-updated by version_bump.py')
        else:
            updated.append(line)
...
def greet(name):
    # Say hello!
    print(f"Hello, {name}!")  # Should see function names, strings, and comments in different colors